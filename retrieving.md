...
The final input, `saveBack` (=1 by default), saves the group indices back to the data file, in this case the **TimeSeries** structure array will be given a new field, which can then be used by a range of other analysis functions (such as `TSQ_plot_pca`).
